...
sensor5 = pd.read_csv('./sensor_5.csv')
force_data = pd.read_csv('./test8L.csv')

# Verify expected columns are present.
print("Sensor 5 columns:", sensor5.columns)
print("Force sensor columns:", force_data.columns)

if 'AccelZ' not in sensor5.columns:
    raise ValueError("The sensor5 data does not contain an 'AccelZ' column.")
...
# Plot sensor5 AccelZ and the resampled force data together.
fig, ax1 = plt.subplots(figsize=(12, 6))
color1 = 'tab:blue'
ax1.set_xlabel("Sample Index")
ax1.set_ylabel("Sensor 5 AccelZ", color=color1)
ax1.plot(sensor5['AccelZ'], color=color1, label='Sensor 5 AccelZ')
...
plt.title("Left Ankle AccelZ and Resampled Test8L Force (250Hz)")
fig.tight_layout()
plt.show()
